...
export default function EditBorrower() {
  ...
  const { id } = useParams();
  const { borrowers, updateBorrower } = useData();

  const existing = borrowers.find((b) => b.id === id) || {};
  ...
  useEffect(() => {
    if (existing) {
      setForm({
        ...
        mobile2: existing.mobile2 || existing.altMobile || "",
        shopWork: existing.shopWork || "",
  ...
  async function handleSave() {
    ...
    try {
      if (updateBorrower) {
        await updateBorrower(id, form);
        navigate(`/borrowers/${id}`);
      } else {
        alert("Update not available");
      }
    } catch (err) {
      alert(err.message || "Unable to update borrower. Please try again.");
    }
  }
  ...
  return (
    <div className="app-shell">
      <header className="flex items-center gap-3 px-4 py-4 bg-white border-b border-gray-200 sticky top-0 z-20">
        ...
        <h1 className="text-2xl font-bold text-primary">Edit Borrower</h1>
      </header>
